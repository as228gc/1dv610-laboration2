...
I have used data structures to represent the summaries used in the report. I created a SummaryDTO data structure to hold summaries of expenses and income by category, which is then returned by the summarizer. The CategorySummarizer is implemented as an object and encapsulates the behavior for summarizing transactions into these structures. By separating objects and data structures, I made sure that manipulation on the objects happens within objects themselves, while data structures are used for holding information. For example, the addAmountToCategory method in CategorySummarizer processes transactions and adds amounts to the belonging category in the Map. This separation makes the code easier to maintain and reduces the risk of mixing responsibilities.
...
I have tried to keep clear boundaries in the system to make it easier to change and maintain. For example, in the TransactionProcessor, methods like filterByCategory handle the logic for selecting transactions based on their category. This keeps the filtering logic in one place, making it easier to update or change later if needed. I have also tried to keep the system flexible by making sure the different parts work independently. I think the boundaries are fine, but there is still room for improvement.
...
### Chapter 11: Systems
I have divided the system into clear and cohesive modules to make it easier to maintain. For example, ReportGenerator focuses on creating reports, while TransactionProcessor handles the filtering and sorting  of transactions. To reduce coupling, I used dependency injection by passing TransactionProcessor into the ReportGenerator, which makes testing easier. However other components like TransactionCalculator and CategorySummarizer are created directly inside ReportGenerator, which could have been improved by injecting them as well. This would make the system even more flexible. Overall, the modular design keeps the code organized, but there is room to further reduce coupling by managing dependencies better.